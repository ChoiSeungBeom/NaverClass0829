...
//클래스가 다 추상일 때는 인터페이스로 만들면됨
//interface Super{
//	public void drawing();
//}

//다형성 처리시 부모가 하는일이 없을경우 어떻게 해야될까요
//오버라이딩을 위해서 일단 메서드는 만들어야 한다

//추상 클래스의 특징
//1. 일반 메서드와 미완성의 abstract 메서드 둘 다 구현 가능하다
//2. abstract(추상) 클래스는 new로 객체 생성을 할 수 없다
//3. 추상 클래스를 상속받는 클래스는 반드시 추상 메서드를 오버라이드해서 기능을 구현해야만 한다
//4. 만약 추상 메서드를 오버라이드 안할경우 서브클래스도 역시 추상화시켜야만 한다
...
//	}
	//오버라이드를 위해서 {} 부분을 없앤다
	//추상메서드(abstract : 추상화- 미완성적인 메서드라는 의미)
	//class의 멤버메서드 중 한개이상의 추상 메서드가 있는경우
	//반드시 클래스도 추상화 시켜야한다 - 즉 class 앞에 abstract를 붙인다
...
public class Ex3_Inheri {
	public static void draw(SuperColor s) {
		s.parentJob(); //부모가 가지고있는 메소드 호출(오버라이드 x)
		s.draw();
	}
