...
//interface에는 상수와 추상메서드만 구현이 가능하다
//그래서 구분을 할필요가 없기 때문에 final,abstract 단어는 안쓴다
//어차피 백프로 상수와 추상 메서드로만 구성이 되어있기 때문이다.
//interface도 new로 셍성할 수 없다

interface InterA{
	int SCORE=100; //final 상수
	public void play(); //abstract 메서드
	public void study(); //abstract 메서드
	
}
//일반 클래스가 interface를 구현할경우 implements로 구현
