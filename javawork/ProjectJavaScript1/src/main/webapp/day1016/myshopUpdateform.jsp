...
<!-- hidden 값: 브라우저에 보이지는 않지만 폼전달시 반드시 같이 넘겨야 하는 경우
	hidden으로 주는데 form 안에 어디에 있던 상관없다(주로 상단에 넣음)-->
	...
	<table class="table table-bordered" style="width: 300px;">
		...
		<tr>
			...
			<td>
				<input type="text" name="sangpum" class="form-control" placeholder="상품명" required="required" autofocus="autofocus" id="sangpum" value=<%=dto.getSangpum()%>>
			</td>
		</tr>
		...
		<tr>
			...
			<td>
				...
				//정수로 변환
				int a=Integer.parseInt(photoName);
				for(int i=1;i<=34;i++){%>
					<option value="<%=i%>.<%=(i==24?"gif":"jpg")%>"><%=i==a?"selected":" " %>상품<%=i%></option>	
				<%}
				%>
				</select>
		...
		<tr>
			...
			<td>
				<input type="color" name="color" class="form-control" required="required" value="<%=dto.getColor()%>">
			</td>
		</tr>
